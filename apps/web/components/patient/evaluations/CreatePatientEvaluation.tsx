...
const transformKipuItem = (kipuItem: any) => {
  ...
  transformed.fieldType = fieldTypeMap[kipuItem.fieldType] || kipuItem.fieldType;
...
export default function PatientEvaluationCreateModal({ patientId, onSuccess }: PatientEvaluationCreateModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    console.log('[Submit] Template info:', { 
      templateId: template?.id, 
      templateName: template?.name,
      hasEvaluation: !!(template as any)?.evaluation,
      evaluationId: (template as any)?.evaluation?.id,
      patientId 
    });
    
    try {
      // Enhanced ID resolution (same as AI Complete)
      const possibleIds = [
        template?.id,
      ...
      console.log('[Submit] ID resolution analysis:', {
        'template?.id': template?.id,
        'template.evaluation?.id': (template as any)?.evaluation?.id,
        'template.templateId': (template as any)?.templateId,
        'template.evaluationId': (template as any)?.evaluationId,
        'template.evaluation_id': (template as any)?.evaluation_id,
        'template.id': (template as any)?.id,
        possibleIds,
      ...
      console.log('[Submit] Using evaluation ID:', evaluationId);
      ...
      console.log('[Submit] Form data being sent:', {
        patientId,
        evaluationId,
        notes: formState.notes || '',
        itemCount: items.length,
        itemsSample: items.slice(0, 3), // First 3 items for debugging
      ...
      console.log('[Submit] Making API request to /api/kipu/patient_evaluations');
  ...
  const handleAiComplete = async () => {
    console.log('[AI Complete] Button clicked!');
    console.log('[AI Complete] Template info:', { 
      templateId: template?.id, 
      templateName: template?.name,
      hasEvaluation: !!(template as any)?.evaluation,
      evaluationId: (template as any)?.evaluation?.id,
    ...
    const evaluation = (template as any)?.evaluation;
    if (evaluation) {
      console.log('[AI Complete] DETAILED EVALUATION INSPECTION:');
      console.log('[AI Complete] evaluation.id:', evaluation.id);
      console.log('[AI Complete] evaluation.name:', evaluation.name);
      console.log('[AI Complete] evaluation keys:', Object.keys(evaluation));
      
      // Look for any array properties that might contain the items
      const arrayProperties = Object.keys(evaluation).filter(key => Array.isArray(evaluation[key]));
      console.log('[AI Complete] Array properties in evaluation:', arrayProperties);
      
      arrayProperties.forEach(key => {
        const array = evaluation[key];
        console.log(`[AI Complete] evaluation.${key}:`, {
          length: array.length,
          firstItem: array[0] ? {
      ...
      // Check specifically for known evaluation item patterns
      const potentialItemArrays = [
        'evaluationItems', 'items', 'fields', 'questions', 'formFields', 
        'evaluationFields', 'templateItems', 'components', 'elements'
      ];
      
      potentialItemArrays.forEach(key => {
        if (evaluation[key]) {
          console.log(`[AI Complete] Found evaluation.${key}:`, {
            isArray: Array.isArray(evaluation[key]),
            length: Array.isArray(evaluation[key]) ? evaluation[key].length : 'not array',
            type: typeof evaluation[key],
    ...
    // Enhanced ID resolution with multiple fallbacks
    const possibleIds = [
      template?.id,
      (template as any)?.evaluation?.id,
      (template as any)?.templateId,
    ...
    console.log('[AI Complete] ID resolution analysis:', {
      'template?.id': template?.id,
      'template.evaluation?.id': (template as any)?.evaluation?.id,
      'template.templateId': (template as any)?.templateId,
      'template.evaluationId': (template as any)?.evaluationId,
    ...
    if (possibleIds.length === 0) {
      console.error('[AI Complete] No template ID found in any location');
      alert('No template ID found. Please select a template first.');
      return;
    }
    
    const evaluationId = String(possibleIds[0]);
    console.log('[AI Complete] Using evaluation ID:', evaluationId);
    
    setAiCompleting(true);
    try {
      console.log('[AI Complete] Making API request...');
      
      // Call the dedicated AI completion API
      const response = await fetch('/api/ai/complete-evaluation', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          evaluationId: evaluationId,
          patientId: patientId,
          reviewMode: true // Always start in review mode for safety
        })
      });

      console.log('[AI Complete] Response status:', response.status);
      console.log('[AI Complete] Response ok:', response.ok);

      if (!response.ok) {
        const errorData = await response.json().catch(() => ({ error: 'Failed to parse error response' }));
        console.error('[AI Complete] API Error:', errorData);
        throw new Error(errorData.error || `HTTP ${response.status}: Failed to complete evaluation with AI`);
      }

      const result = await response.json();
      console.log('[AI Complete] API Result:', result);
      
      if (result.success && result.data.fieldCompletions) {
        // Populate the form with AI-generated completions
        console.log('[AI Complete] Field completions received:', Object.keys(result.data.fieldCompletions));
        setFormState(result.data.fieldCompletions);
        
        // Log the result for debugging
        console.log('[AI Complete] Successfully populated form:', {
          fieldsCompleted: Object.keys(result.data.fieldCompletions).length,
          draftResponse: result.data.draftResponse,
          reviewNotes: result.data.reviewNotes
        });
        
        // Show success alert for now
        alert(`AI completed ${Object.keys(result.data.fieldCompletions).length} fields. Please review before submitting.`);
      } else {
        console.error('[AI Complete] Invalid result:', result);
        throw new Error('No field completions received from AI');
      }
      
    } catch (error: any) {
      console.error('[AI Complete] Error:', error);
      console.error('[AI Complete] Error stack:', error.stack);
      
      // Show error alert
      alert(`AI completion failed: ${error.message || 'Unknown error'}`);
    } finally {
      console.log('[AI Complete] Setting aiCompleting to false');
      setAiCompleting(false);
    }
  };

  // Handle test form population (for debugging)
  const handleTestPopulate = () => {
    console.log('[Test] Populating form with mock data...');
    console.log('[Test] Available template items:', templateItems.length);
    
    // Create mock data for the first few fields
    const mockData: Record<string, any> = {};
    templateItems.slice(0, 10).forEach((item: any, index: number) => {
      ...
      switch (item.fieldType) {
        ...
        case 'textarea':
        case 'string':
          mockData[fieldId] = `Test response for ${item.label} (field ${index + 1})`;
          break;
        case 'radio_buttons':
          if (item.recordNames) {
            const options = item.recordNames.split('|');
            mockData[fieldId] = options[0]; // Select first option
          }
          break;
        case 'check_box':
        case 'check_box_first_value_none':
          if (item.recordNames) {
            const options = item.recordNames.split('|');
            mockData[fieldId] = [options[0]]; // Select first option as array
          }
          break;
        case 'evaluation_datetime':
        case 'datetime':
        case 'date':
          mockData[fieldId] = new Date().toISOString().slice(0, 16); // Current datetime
          break;
        default:
          mockData[fieldId] = `Test value ${index + 1}`;
      }
    });
    
    console.log('[Test] Mock data created:', mockData);
    setFormState(mockData);
    alert(`Test: Populated ${Object.keys(mockData).length} fields with mock data`);
  };
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="max-w-4xl max-h-[85vh] overflow-y-auto">
        ...
        {!template ? (
          ...
          <div>
            {/* AI Complete Button - Show when template is selected and has fields */}
            {templateItems.length > 0 && (
              <div className="mb-4 p-3 bg-gradient-to-r from-blue-50 to-indigo-50 border border-blue-200 rounded-lg">
                <div className="flex items-center justify-between">
                  <div>
                    <h4 className="text-sm font-medium text-blue-900">AI Auto-Complete</h4>
                    <p className="text-xs text-blue-700 mt-1">
                      Let AI analyze patient context and complete this evaluation automatically
                    </p>
                  </div>
                  <div className="flex gap-2">
                    ...
                    <Button 
                      ...
                      disabled={aiCompleting || isLoadingKipuTemplates}
